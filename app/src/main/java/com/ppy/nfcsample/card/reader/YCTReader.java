...
public class YCTReader extends BaseReader implements IReader {
    ...
    public DefaultCardInfo readCard(NfcCardReaderManager mReaderManager) throws IOException {
        ...
        Iso7816 dir_pse = new Iso7816(Commands.selectByName(Commands.JIAOTONG_PSE));
        ...
        Iso7816 select = new Iso7816(Commands.selectByName(Commands.JIAOTONG_SELEFILE));
...
//        }
        //parse balance
        boolean parseBalance = cardInfo.parseCardBalance(cmds.get(1).getResp());
        if (!parseBalance) {
            return null;
        }
        //read record
